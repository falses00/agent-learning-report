...
这不是完整生产 Agent，而是课程 F0、S0-S4 的可运行起点。它用确定性组件稳定证明：HTTP/CLI/SQLite 基础、严格契约、租户隔离、ToolCall 只是申请、高风险工具审批、operation ledger 幂等、多租户 RAG、可信引用、拒答、崩溃恢复、provider reconciliation 和组合式 eval assertions。
...
- 三套 eval 分别报告 case、critical failure 和 assertion 统计。
...
真实模型会引入随机性、网络、限流、费用和 provider 差异。F0、S0-S4 先固定 planner、lexical retrieval 和 mock refund provider，可以稳定复现权限、状态、副作用、引用、拒答和恢复故障。后续添加 `ModelGateway`、embedding、reranker 或 workflow adapter 时，仍保留 deterministic baseline 作为回归 fixture。
...
因此该代码只能作为 F0、S0-S4 教学证据，不能接入真实资金、客户数据或生产凭据。
